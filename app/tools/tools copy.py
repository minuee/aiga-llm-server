...
sql_agent_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            - hospital.hid = hospital_evaluation.hid
            - doctor.did =  doctor_evaluation.did
...
async def search_doctor_for_else_question(question: str) -> dict:
    ...
    try:
        # SQL agent를 직접 호출. 사용자 질문만 간단히 전달
        ...
        # Agent의 결과가 dict가 아닐 수 있음 (문자열 직접 반환 등)
        if isinstance(result, dict):
            text_response = result.get("output", "답변을 찾을 수 없습니다.")
        else:
        ...
        logger.info(f"DEBUG: search_doctor_for_else_question is returning: {return_value}")
        return return_value
        ...
        logger.error(f"DEBUG: search_doctor_for_else_question is returning an error: {error_return_value}")
