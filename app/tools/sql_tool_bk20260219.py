...
def handle_proximity_search(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        if is_location_near:
            ...
            # '근처' 검색 시 좌표를 찾지 못하면 사용자에게 더 정확한 주소를 요구
            if not coords_for_distance:
                logger.warning(f"근처 검색을 위한 좌표를 찾지 못했습니다: location='{location}'")
                # chat_type을 'general'로 수정
                return {"chat_type": "general", "message": f"입력하신 '{location}'의 위치를 찾을 수 없습니다. 더 정확한 주소(예: 시/도, 시/군/구 포함)를 알려주시겠어요?"}
...
async def search_hospitals_by_location_and_department(department: Union[str, List[str]], location: Optional[str] = None, latitude: Optional[float] = None, longitude: Optional[float] = None, is_location_near: bool = False, coords_for_distance: Optional[dict] = None, limit: Optional[int] = None) -> dict:
    ...
    # --- User's new rule implementation ---
    department_list = department if isinstance(department, list) else [department]
    
    department_search_term = ""
    if len(department_list) == 1:
        term = department_list[0]
        if term and term.strip():
            department_search_term = f"+{term.strip()}"
    elif len(department_list) > 1:
        department_search_term = " ".join(d.strip() for d in department_list if d and d.strip())
    # --- End of new rule implementation ---

    department_where_clause = ""
    if department_search_term:
        escaped_department = escape_string_for_sql(department_search_term)
        department_where_clause = f"AND MATCH(db.deptname) AGAINST('{escaped_department}' IN BOOLEAN MODE)"
    else:
        # 부서 정보가 없으면 검색 의미 없음
    ...
    logger.info(f"Generated SQL Query: {template_query}")
...
async def search_doctors_by_location_and_department(department: Union[str, List[str]], location: Optional[str] = None, latitude: Optional[float] = None, longitude: Optional[float] = None, is_location_near: bool = False, coords_for_distance: Optional[dict] = None, limit: Optional[int] = None, proposal: str = "") -> dict:
    ...
    params = {}
    if department_search_term:
        params['department'] = department_search_term
        department_where_clause = "AND MATCH(db.deptname) AGAINST(:department IN BOOLEAN MODE)"
    else:
        return {"chat_type": "search_doctor", "answer": {"doctors": []}}
...
async def search_doctors_by_disease_and_location(disease: Union[str, List[str]], location: Optional[str] = None, latitude: Optional[float] = None, longitude: Optional[float] = None, is_location_near: bool = False, coords_for_distance: Optional[dict] = None, limit: Optional[int] = None, proposal: str = "") -> dict:
    ...
    if len(disease_list) == 1:
        term = disease_list[0]
        if term and term.strip():
            disease_search_term = f"+{term.strip()}"
    elif len(disease_list) > 1:
        disease_search_term = " ".join(d.strip() for d in disease_list if d and d.strip())
    # --- End of new rule implementation ---

    disease_where_clause = ""
    params = {}
    if disease_search_term:
        params['disease'] = disease_search_term
        disease_where_clause = "AND MATCH(db.parse_specialties) AGAINST(:disease IN BOOLEAN MODE)"
    else:
        return {"chat_type": "search_doctor", "answer": {"doctors": []}}
...
async def search_hospital_by_disease_and_location(disease: Union[str, List[str]], location: Optional[str] = None, latitude: Optional[float] = None, longitude: Optional[float] = None, is_location_near: bool = False, coords_for_distance: Optional[dict] = None, limit: Optional[int] = None) -> dict:
    ...
    disease_where_clause = ""
    if disease_search_term:
        escaped_disease = escape_string_for_sql(disease_search_term)
        disease_where_clause = f"AND MATCH(db.parse_specialties) AGAINST('{escaped_disease}' IN BOOLEAN MODE)"
    else:
        return {"chat_type": "recommand_hospital", "answer": {"hospitals": []}}
...
async def search_hospital_by_disease_and_department(disease: Union[str, List[str]], department: Union[str, List[str]], location: Optional[str] = None, latitude: Optional[float] = None, longitude: Optional[float] = None, is_location_near: bool = False, coords_for_distance: Optional[dict] = None, limit: Optional[int] = None) -> dict:
    ...
    department_list = l_department if isinstance(l_department, list) else [l_department]
    ...
    clauses = []
    if disease_search_term:
        clauses.append(f"MATCH(db.parse_specialties) AGAINST('{escape_string_for_sql(disease_search_term)}' IN BOOLEAN MODE)")
    if department_search_term:
        clauses.append(f"MATCH(db.deptname) AGAINST('{escape_string_for_sql(department_search_term)}' IN BOOLEAN MODE)")

    if not clauses:
        logger.warning("질환과 진료과 검색어가 모두 비어있어, 빈 병원 목록을 반환합니다.")
        return {"chat_type": "recommand_hospital", "answer": {"hospitals": []}}
    ...
    logger.info(f"최종 생성된 WHERE 조건: {combined_where_clause}")
