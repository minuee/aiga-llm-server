    ...
    if classification == "NAMED_LOCATION" and anchor_noun:
        ...
        # 3b: 모호하지 않은 신규 위치 (하지만 시/도 정보가 필요할 수 있음)
            ...
            if normalized_sido:
                ...
                prompt = f"""주어진 "문장"의 문맥 안에서 "단어"가 지리적 위치(시/도, 시/군/구, 읍/면/동 등)를 찾으려는 의도로 사용되었는지 판단해주세요.

**[판단 주의사항]**
1. "단어"가 대한민국에 존재하는 지명이더라도, 문맥상 사람의 이름(인명)의 일부이거나 사람을 지칭하는 경우 {{"is_location": false}}로 판단해야 합니다.
2. 사용자가 의사나 교수를 찾고 있는 문맥이라면, 해당 단어는 위치가 아닐 가능성이 매우 높습니다.
3. 단어가 3글자 이상인 이름(예: 문경근)에서 앞의 두 글자(문경)만 추출된 경우라면 반드시 false를 주어야 합니다.

대답은 JSON 형식으로 {{"is_location": true}} 또는 {{"is_location": false}} 중 하나로만 반환해주세요.
대한민국 영토 내의 지리적 위치만 true에 해당하며, 외국 지명일 경우 {{"is_location": false, "is_national": true}}를 반환하세요.
{prompt_rules}
...
문장: "뉴욕에 있는 병원 알려줘"
단어: "뉴욕"
대답: {{"is_location": false, "is_national": true}}
---
[실제 작업]
문장: "{user_message}"
단어: "{anchor_noun}"
