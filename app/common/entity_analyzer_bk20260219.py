...
async def extract_entities_for_routing(llm: AzureChatOpenAI, state: dict) -> dict:
    ...
    prompt = f"""아래 대화 기록과 현재까지 추출된 엔티티 정보 (conversation history 및 persistent entity_history에서 통합됨)를 모두 참고하여 'location', 'disease', 'department', 'target'('의사' 또는 '병원')를 추출합니다.
...
6. ** 어떤 경우에도 'target'은 null이 될 수 없으며, 반드시 '의사' 또는 '병원' 중 하나를 선택해야 한다.**
   - **대화 기록을 참고하여 설정한다.
[예시]
대화 기록:
