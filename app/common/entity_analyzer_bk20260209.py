...
async def extract_entities_for_routing(llm: AzureChatOpenAI, state: dict) -> dict:
    ...
    # LLM 프롬프트에 미리 추출된 엔티티 정보와 entity_history를 포함하여 전달
    prompt = f"""아래 대화 기록과 현재까지 추출된 엔티티 정보 (conversation history 및 persistent entity_history에서 통합됨)를 모두 참고하여 'location', 'disease', 'department', 'target'('의사' 또는 '병원'), 'target_reason'을 추출합니다.
특히, 미리 추출된 엔티티 정보가 있다면, 이를 우선적으로 활용하고 보완하여 최종 결정을 내리세요.

[현재까지 추출된 엔티티 (integrated from conversation history and persistent entity_history)]
- Diseases: {current_extracted['diseases']}
- Departments: {current_extracted['departments']}
- Hospitals: {hospital_names}
- Doctors: {[d['name'] for d in current_extracted['doctors'] if d.get('name')] if current_extracted['doctors'] else []}
- Location: {current_extracted['location']}

[규칙]
1. **매우 중요: '소아과', '내과', '심장내과'와 같은 진료과목 이름은 절대로 'disease' 필드에 넣지 말라. 'disease' 필드는 '감기', '고혈압', '당뇨'와 같은 실제 질병이나 증상 이름만 포함해야 한다. 진료과목은 'department' 필드에만 해당된다.**
...
2. '내 근처', '여기 근처' 등 사용자 자신을 기준으로 하는 단어는 'location'이 아니다. 이런 단어가 보이면 'location'은 null로 설정해라. 최신 순의 대화 흐름을 판단하여 location을 추출해야 한다. 병원정보의 해당하는 address파마리터를 절대 참고하지 말라.
3. 2번의 경우 휴먼메시지가 아닌 AI가 답변한 메시지 내에서 가령 "현재 위치 근처에서" 등의 사용자 자신을 기준으로 하는 단어는 'location'을 null로 설정해라.
4. 대화 기록에서 이미 '시/도' 정보(예: 서울)가 언급되었고, 마지막 질문에 '구/동' 정보(예: 중구)만 있다면, 이 둘을 조합하여 '서울 중구'와 같이 완전한 지역명을 'location'으로 추출해야 한다.
5. **대화의 전체 맥락을 고려하세요. 사용자의 마지막 질문에 특정 정보(예: 진료과)가 없다면, 이전 대화들에서 해당하는 가장 최신 정보를 찾아 사용해야 한다.**
6. **'target' 결정은 다음 규칙을 최우선 순서로 따른다. 어떤 경우에도 'target'은 null이 될 수 없으며, 반드시 '의사' 또는 '병원' 중 하나를 선택해야 한다.**
   - **1순위:** 마지막 사용자 질문에 '의사', '명의', '전문의'가 있으면, **다른 모든 것을 무시하고** 'target'을 **'의사'** 로 설정한다.
   - **2순위:** 1순위가 아니고, 마지막 사용자 질문에 '병원'이 있으면 'target'을 '병원'으로 설정한다.
   - **3순위:** 1, 2순위가 아니면, 이전 대화에서 마지막으로 추천된 것이 의사인지 병원인지에 따라 'target'을 결정한다.
   - **4순위 (필수 폴백):** 위의 모든 조건으로도 'target'을 결정할 수 없다면, **무조건 '의사'** 로 설정해야 한다.
   - ** target_reason를 반드시 서술형으로 작성한다. 판단근거를 제시해라 
[예시]
대화 기록:
...
"""
    try:
        ...
        # is_only_target_from_llm 확인을 위해 LLM 추출 값만 있는 상태에서 먼저 확인
        is_only_target_from_llm = (
            final_entities.get("target") is not None and
            not final_entities.get("diseases") and
            not final_entities.get("department") and
            not final_entities.get("location")
        )

        if is_only_target_from_llm:
            entity_history = state.get("entity_history", {"hospitals": [], "doctors": [], "departments": [], "diseases": [], "location": None})
            logger.info("Entities for routing: Only target found from LLM. Supplementing ALL fields from entity_history.")
            # entity_history에서 모든 필드 값을 가져와 리스트에 추가
            _add_unique_items(final_entities["diseases"], entity_history.get("diseases"))
            _add_unique_items(final_entities["department"], entity_history.get("departments"))
            _add_unique_items(final_entities["hospitals"], entity_history.get("hospitals"))
            _add_unique_doctors(final_entities["doctors"], entity_history.get("doctors"))
            if not final_entities["location"] and entity_history.get("location"): # location이 없을 경우에만 history에서 가져옴
                final_entities["location"] = entity_history["location"]
        ...
        # 1. department가 최종적으로 있으면 다른 엔티티 초기화 (location, target 제외)
        if final_entities["department"]:
            logger.info(f"Entities for routing: final department found: {final_entities['department']}. Initializing other fields based on priority.")
            final_entities["diseases"] = [] # 'diseases' 리스트를 비움
            final_entities["hospitals"] = []
            final_entities["doctors"] = []

        # 2. department는 없고 hospital이 최종적으로 있으면 다른 엔티티 초기화 (location, target 제외)
        elif not final_entities["department"] and final_entities["hospitals"]:
            logger.info(f"Entities for routing: final hospital found: {final_entities['hospitals']}. Initializing other fields based on priority.")
            final_entities["diseases"] = [] # 'diseases' 리스트를 비움
            final_entities["doctors"] = []

        # 3. department, hospital은 없고 disease가 최종적으로 있으면 다른 엔티티 초기화 (location, target 제외)
        elif not final_entities["department"] and not final_entities["hospitals"] and final_entities["diseases"]: # 'diseases'로 변경
            logger.info(f"Entities for routing: final diseases found: {final_entities['diseases']}. Initializing other fields based on priority.")
            final_entities["doctors"] = []
