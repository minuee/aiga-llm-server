...
async def agent_node(state: AgentState, config: RunnableConfig):
    ...
    # 기존 LLM_SAVE_ENTRY_HISTORY_VERBOSE 로직은 제거하고 이 로직으로 대체
...
"""
    ...
    # Log the full injected info (truncated for brevity)
    #   logger.info(f"Injected System Prompt Context (truncated to 1000 chars): {(injected_info + persistent_facts_info)[:1000]}{'...' if len(injected_info + persistent_facts_info) > 1000 else ''}")
    ...
    # persistent_facts_info는 이미 위에서 계산됨
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage):
                try:
                    ...
                    if isinstance(content_data, dict) and content_data.get("migrated") is True:
                        result_id = content_data.get("result_id")
                        if result_id:
                            #logger.info(f"Attempting to restore ToolMessage with result_id: {result_id} for session_id: {session_id}")
                            async with conn.cursor() as cursor:
                                ...
                                if row:
                                    ...
                                    msg.content = original_content # 원본 내용으로 복원
                                    #logger.info(f"ToolMessage (result_id: {result_id}) restored for session {session_id}.")
                                else:
                                    logger.warning(f"No cached content found for result_id: {result_id} in session {session_id}.")
                except (json.JSONDecodeError, TypeError):
                    # content가 유효한 JSON이 아니거나 dict가 아니면 무시
                    pass
    finally:
        await conn.close()

    # ToolMessage 마이그레이션 로직 (이전과 동일)
    if settings.llm_summary_verbose:
        session_id = config["configurable"]["thread_id"]
        conn = await aiosqlite.connect(settings.sqlite_directory, check_same_thread=False)

        # 현재 턴에서 방금 실행된 최신 ToolMessage는 마이그레이션에서 제외한다.
        # 최신 ToolMessage는 보통 리스트의 마지막에 위치하며, 그 앞에는 tool_calls를 가진 AIMessage가 있다.
        ...
        for i in range(len(messages) - 1, -1, -1):
            ...
            if isinstance(msg, ToolMessage):
                try:
                    ...
                    logger.info(f"Attempting to migrate ToolMessage, assigning new result_id: {result_id} for session_id: {session_id}")
                    async with conn.cursor() as cursor:
                        await cursor.execute(
                    ...
                    if 'chat_type' in tool_content_json:
                        ...
                        param_dict = {} # param_dict는 이 시점에 초기화

                        if isinstance(answer_content, dict):
                            summary_parts = []
                            count_info = "" # count_info도 이 안에서 정의
                            ...
                            # 3. 병원명 (hospital) - 단일 또는 대표
                            if answer_content.get('hospital'): # 단일 병원 정보가 직접 있는 경우
                                summary_parts.append(f"병원: {answer_content['hospital']}")
                                param_dict['hospital'] = answer_content['hospital']
                            elif answer_content.get('hospitals') and len(answer_content['hospitals']) > 0: # 병원 목록이 있는 경우 대표 병원명
                                first_hospital_name = answer_content['hospitals'][0].get('name', '')
                                if first_hospital_name:
                                    summary_parts.append(f"주요 병원: {first_hospital_name}")
                                    param_dict['hospital'] = first_hospital_name
                                param_dict['hospital_count'] = len(answer_content['hospitals'])
                            
                            # 4. 의사명 (name) - 대표 의사명 (예: 첫 번째 의사)
                            if answer_content.get('doctors') and len(answer_content['doctors']) > 0:
                                first_doctor_name = answer_content['doctors'][0].get('name', '')
                                if first_doctor_name:
                                    summary_parts.append(f"주요 의사: {first_doctor_name}")
                                    param_dict['doctor'] = first_doctor_name
                                param_dict['doctor_count'] = len(answer_content['doctors'])
                            
                            # 결과 개수 (param_dict에 먼저 추가 후 summary_parts와 합침)
                            if answer_content.get('doctors'):
                                count_info = f"{len(answer_content['doctors'])}명의 의사 정보"
                            elif answer_content.get('hospitals'):
                                count_info = f"{len(answer_content['hospitals'])}개의 병원 정보"

                            if summary_parts or count_info: # 어떤 정보라도 있으면 요약
                                placeholder_summary = f"과거 {tool_content_json['chat_type']} 결과: {count_info}{' (' + ', '.join(summary_parts) + ')' if summary_parts else ''}"
                            else: # 아무 정보도 없으면 기본 요약
                                placeholder_summary = f"과거 {tool_content_json['chat_type']} 결과 (상세 정보 없음)"

                        elif isinstance(answer_content, str):
                            placeholder_summary = f"과거 {tool_content_json['chat_type']} 결과: {answer_content[:100]}... (저장됨)"
                            param_dict = {} # 문자열 요약인 경우 param_dict는 비워둠
                    else: # chat_type이 없을 경우
                        param_dict = {} # param_dict 초기화
                        placeholder_summary = "과거 도구 실행 결과가 외부에 저장되었습니다." # chat_type이 없는 경우 기본 요약
                    msg.content = json.dumps({
                        "migrated": True,
                        "result_id": result_id,
                        "summary": placeholder_summary,
                        "param": param_dict # 새로 추가: param_dict 내용 추가
                    }, ensure_ascii=False)
                    logger.info(f"Generated placeholder_summary: {msg.content}")
                    logger.info(f"Older ToolMessage migrated for session {session_id}, result_id: {result_id}")
                except json.JSONDecodeError:
                    pass
                except Exception as e:
                    logger.error(f"Error during older ToolMessage migration for session {session_id}: {e}", exc_info=True)
        await conn.close()
    ...
    for i, msg in enumerate(messages):
        # 마지막 HumanMessage까지는 무조건 유지
        if i <= last_human_idx:
            cleaned_messages.append(msg)
        # 마지막 HumanMessage 이후의 메시지 처리
            ...
            if not is_error_message:
                ...
                logger.info(f"Removed previous error AIMessage from context (after last HumanMessage): {msg.content}")
        else: # SystemMessage, ToolMessage 등은 그대로 유지
            cleaned_messages.append(msg)
            
    state["messages"] = cleaned_messages
    messages = state["messages"] # 업데이트된 메시지 리스트를 사용하도록 재할당
    # 🚨 END: 에러 AIMessage 제거 로직

    # --- START: Try-Catch-Retry 로직 ---
    try:
        # 1. 원본 메시지로 첫 번째 시도
        logger.info("Calling model with original message...")
        #logger.info(f"\n--- LLM INPUT DEBUG START (Truncated) ---")
        #for i, msg in enumerate(messages):
        #    truncated_content = (msg.content[:500] + '...[TRUNCATED]') if isinstance(msg.content, str) and len(msg.content) > 500 else msg.content
        #    logger.info(f"  [{i}] Type: {type(msg).__name__}, Content: {truncated_content!r}")
        #logger.info(f"--- LLM INPUT DEBUG END (Truncated) ---")
        ...
        # --- START: tool_calls 관련 invalid_request_error 처리 ---
        # "tool_calls" 관련 에러 메시지를 식별합니다.
        if "An assistant message with 'tool_calls' must be followed by tool messages" in error_message or \
           ...
           ("invalid_request_error" in error_message and "tool_calls" in error_message):
            ...
            logger.error(f"Detected invalid_request_error related to tool_calls. Attempting to clear tool_calls from last AIMessage to continue conversation.")
            ...
            if last_ai_message_idx != -1:
                # 해당 AIMessage의 tool_calls를 비우고 사용자에게 알림 메시지를 content에 추가
                if state["messages"][last_ai_message_idx].tool_calls:
                    logger.info(f"Clearing tool_calls from AIMessage at index {last_ai_message_idx}.")
                    state["messages"][last_ai_message_idx].tool_calls = [] # tool_calls를 빈 리스트로 설정
                    state["messages"][last_ai_message_idx].content = "처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."
                    state["last_ai_message"] = state["messages"][last_ai_message_idx].content # last_ai_message도 업데이트
                else:
                    # tool_calls 필드는 비어있지만, 에러가 발생한 경우 (매우 드뭄)
                    state["messages"][last_ai_message_idx].content = "처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."
                    state["last_ai_message"] = state["messages"][last_ai_message_idx].content
            else:
                # `messages`에 AIMessage가 전혀 없는 상태에서 이 에러가 발생한 경우 (극히 드묾)
                state["messages"].append(AIMessage(content="처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."))
                state["last_ai_message"] = "처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."

            # 이 에러 처리 후에는 `agent_node`의 정상 반환 경로를 따르도록 합니다.
            # `should_continue` 함수가 `tool_calls`가 없는 것을 확인하고 `validate` 노드로 이동할 것입니다.
            return {
                "messages": state["messages"],
                "retry": state.get("retry", 0), # retry 값 유지
                "location_history": state['location_history'],
                "entity_history": state['entity_history'],
                "last_ai_message": state["last_ai_message"]
            }
        # --- END: tool_calls 관련 invalid_request_error 처리 ---
        ...
        if "content management policy" in error_message or "content filter" in error_message:
            ...
            logger.info(f"Generated sanitized content for retry: {sanitized_content}")
            ...
            try:
                # 4. 정제된 메시지로 재시도
                logger.info("Retrying LLM call with sanitized message...")
                response = await model.ainvoke(sanitized_messages_for_llm, config)
            except Exception as retry_e:
                # 5. 재시도도 실패하면 최종 에러 처리
                logger.error(f"Sanitized message retry also failed: {retry_e}. Returning fallback message.")
                retry_error_message = str(retry_e)
                if "The response was filtered due to the prompt triggering Azure OpenAI's content management policy" in retry_error_message or \
                   "Azure has not provided the response due to a content filter being triggered" in retry_error_message:
                    # 재시도 실패가 콘텐츠 필터 때문일 경우의 특정 메시지
                    fallback_message = "죄송합니다. AI 콘텐츠 필터링 정책으로 답변이 일시 중단되었습니다. 표현을 바꿔 다시 질문해주세요."
                else:
                    # 다른 종류의 재시도 실패일 경우의 일반 메시지
                    fallback_message = "처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."
                response = AIMessage(content=fallback_message)
        else:
            # 6. 콘텐츠 필터가 아닌 다른 에러일 경우
            logger.error(f"A non-filter error occurred: {e}. Returning fallback message.")
            fallback_message = "처리 중 문제가 발생하여 요청을 완료하지 못했습니다. 불편을 드려 죄송합니다. 다시 시도해 주시거나 다른 질문을 해주세요."
            response = AIMessage(content=fallback_message)
    # --- END: Try-Catch-Retry 로직 ---

    # LLM이 직접 생성한 답변에 인사말 강제 삽입
    # num_interactions <= 1이고, 응답 내용이 '안녕하세요! AIGA입니다.'로 시작하지 않는 경우에만 추가
    # LLM이 직접 생성한 답변에 인사말 강제 삽입
    # num_interactions <= 1에서 is_first_interaction_in_session으로 변경
    # 동적 언어 규칙 주입 (이전과 동일)
    locale = state.get("locale") or "ko"
    greeting = LANGUAGE_GREETINGS.get(locale, DEFAULT_GREETING)
    ...
    state["last_ai_message"] = response.content # LLM의 최종 응답 저장
    ...
    return {
        ...
        "location_history": state['location_history'],
        "entity_history": state['entity_history'],
        "last_ai_message": state["last_ai_message"] # state 반환 시 포함
    }
